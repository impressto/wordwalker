...
echo "   - service-worker.js (ROOT - must be in /wordwalker/ for scope)"
echo "   - package.json (for version cache busting)"
echo "   - dist/ folder (entire folder)"
echo "   - pwa-test.html (optional: PWA debugging tool)"
echo "   - sw-debug.html (optional: advanced debugging tool)"
echo ""

# Check version consistency
PKG_VERSION=$(grep -oP '"version":\s*"\K[^"]+' package.json)
SW_VERSION=$(grep -oP "CACHE_NAME = 'wordwalker-v\K[^']+" service-worker.js 2>/dev/null || echo "not found")
...
grep -A 3 "const CACHE_NAME" service-worker.js 2>/dev/null || echo "   Could not read service-worker.js"
echo ""

echo "🔍 After deploying, test at:"
echo "   https://impressto.ca/wordwalker/sw-debug.html (recommended)"
...
echo "   1. Visit https://impressto.ca/wordwalker/sw-debug.html"
echo "   2. Click 'Register Service Worker' button"
echo "   3. Check that all critical files are cached (green checkmarks)"
echo "   4. Visit https://impressto.ca/wordwalker/index.php"
echo "   5. Click 'Install App' button (or use browser menu)"
echo "   6. Play until some assets are cached"
echo "   7. Turn off internet (airplane mode)"
echo "   8. Open PWA - should work offline!"
echo ""
echo "💡 Quick checks:"
...
echo "   - Service worker is NOW in ROOT /wordwalker/service-worker.js"
echo "   - This allows it to control the /wordwalker/ scope"
echo "   - index.php registers it from service-worker.js (relative path)"
